...
@Service
public class Customer AgreementRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Customer AgreementRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.customer agreement.hosturl}")
	private String url;
	
	@Value("${spring.api.customer agreement.context-path}")
	private String contextPath;

	@Value("${spring.api.customer agreement.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Customer AgreementRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Customer AgreementRestProcessor.class.getName());
