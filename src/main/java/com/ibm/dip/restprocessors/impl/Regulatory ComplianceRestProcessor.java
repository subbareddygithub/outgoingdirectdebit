...
@Service
public class Regulatory ComplianceRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(Regulatory ComplianceRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.regulatory compliance.hosturl}")
	private String url;
	
	@Value("${spring.api.regulatory compliance.context-path}")
	private String contextPath;

	@Value("${spring.api.regulatory compliance.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  Regulatory ComplianceRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ Regulatory ComplianceRestProcessor.class.getName());
